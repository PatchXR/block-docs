# Set Material

**Category**: 🎨 Visual

![set_material thumbnail](https://portal.patchxr.io/block-thumbnails/set_material.png)

## Description

Let you change the texture and materials to other block.
Drag and drop from the ball to another block to change its look.

You can also select which mesh inside the block is affected, change colors, use snapshot block to make new textures...

## Inputs, Outputs and Parts

**Previous**: Previous material in library (will cancel your changes)

**Next**: Next material in library (will cancel your changes)

**SelectionShader**: 

**Block Select**: Drag and drop to any block to change its material.

**Mesh Select**: Toggle which meshes of the block will be affected.

**Close**: Material we still remain hidden, can be displayed again using the Material Brush.

**Spawn Brush**: Spawn a brush to apply your material to objects.

---

*Last updated: 2026-01-02 06:30*